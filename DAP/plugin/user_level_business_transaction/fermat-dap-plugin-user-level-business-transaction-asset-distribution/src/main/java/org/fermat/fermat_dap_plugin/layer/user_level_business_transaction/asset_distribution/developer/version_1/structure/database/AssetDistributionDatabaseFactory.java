 ...
 */
public final class AssetDistributionDatabaseFactory {

    private final PluginDatabaseSystem pluginDatabaseSystem;
    private final UUID pluginId;

    public AssetDistributionDatabaseFactory(PluginDatabaseSystem pluginDatabaseSystem, UUID pluginId) {
    ...
    public Database createDatabase() throws CantCreateDatabaseException {
        Database database;
        try {
            database = this.pluginDatabaseSystem.createDatabase(pluginId, AssetDistributionDatabaseConstants.ASSET_DISTRIBUTION_DATABASE);
        ...
        try {
            ...
            digitalAssetMetadataTable = databaseFactory.newTableFactory(pluginId, AssetDistributionDatabaseConstants.ASSET_DISTRIBUTION_TABLE);
            digitalAssetMetadataTable.addColumn(AssetDistributionDatabaseConstants.ASSET_DISTRIBUTION_ID_COLUMN_NAME, DatabaseDataType.STRING, 100, Boolean.TRUE);

            digitalAssetMetadataTable.addIndex(AssetDistributionDatabaseConstants.ASSET_DISTRIBUTION_ID_COLUMN_NAME);
            try {
                //Create the table
                databaseFactory.createTable(pluginId, digitalAssetMetadataTable);
            } catch (CantCreateTableException cantCreateTableException) {
                throw new CantCreateDatabaseException(CantCreateDatabaseException.DEFAULT_MESSAGE, cantCreateTableException, "Creating " + AssetDistributionDatabaseConstants.ASSET_DISTRIBUTION_TABLE + " table", "Exception not handled by the plugin, There is a problem and I cannot create the table.");
            }

            DatabaseTableFactory eventsRecorderTable = databaseFactory.newTableFactory(pluginId, AssetDistributionDatabaseConstants.ASSET_DISTRIBUTION_EVENTS_RECORDED_TABLE_NAME);
    ...
    public boolean databaseExists() {
        try {
            pluginDatabaseSystem.openDatabase(pluginId, AssetDistributionDatabaseConstants.ASSET_DISTRIBUTION_DATABASE);
            return true;
